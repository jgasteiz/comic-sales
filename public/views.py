...
def sale_list(request):
    ...
    context = {"sale_list": Sale.objects.filter(date_end__gte=today)}
...
def api_all_sales(request):
    ...
    all_sales = [
        f"{s.title}, ends on {s.date_end.strftime('%B %d')}" for s in comic_sales
    ]
...
def api_last_sales(request):
    today = timezone.datetime.today().date()
    comic_sales = Sale.objects.filter(
        date_end__gte=today, created_at__gte=today - datetime.timedelta(days=7)
    )
    last_sales = [
        f"{s.title}, ends on {s.date_end.strftime('%B %d')}" for s in comic_sales
    ]
